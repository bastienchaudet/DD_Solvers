function A = A2d_EtaMinusDelta(eta,xmin,xmax,ymin,ymax,Jx,Jy,BCtype,pl,pr,pb,pt)
...
% Omega=(xmin,xmax)x(ymin,ymax)using Jx x Jy points.
    ...
    % Initialization
    h = (xmax-xmin)/(Jx-1);
    % eta is replaced by eta/2 to ensure the validity of tensor formulas
    eta2d = eta/2;
    
    % Dirichlet case
    if (BCtype == 0)
        Dxx = A1d_EtaMinusDelta(eta2d,xmin,xmax,Jx,0,0,0);
        Dyy = A1d_EtaMinusDelta(eta2d,ymin,ymax,Jy,0,0,0);
        A = kron(speye(size(Dxx)),Dyy)+kron(Dxx,speye(size(Dyy)));
        
    % Robin case
    else
        % Initialize standard FD matrices of size (Jx x Jy)
        Dxx = A1d_EtaMinusDelta(eta2d,xmin-h,xmax+h,Jx+2,0,0,0);
        Dyy = A1d_EtaMinusDelta(eta2d,ymin-h,ymax+h,Jy+2,0,0,0);
        ...
        % Build A
        A = kron(Ix,Dyy)+kron(Hxl,Pl)+kron(Hxr,Pr)+kron(Dxx,Iy)+kron(Pb,Hyb)+kron(Pt,Hyt);
        
        % Correct corner indices
        % Bottom-left corner
        A(1,1) = 0.25*(eta+4/h^2)+0.5*(pl(1)+pb(1))/h;
        % Bottom-right corner
        A(1+(Jx-1)*Jy,1+(Jx-1)*Jy) = 0.25*(eta+4/h^2)+0.5*(pr(1)+pb(end))/h;
        % Top-left corner
        A(Jy,Jy) = 0.25*(eta+4/h^2)+0.5*(pl(end)+pt(1))/h;
        % Top-right corner
        A(end,end) = 0.25*(eta+4/h^2)+0.5*(pr(end)+pt(end))/h;
