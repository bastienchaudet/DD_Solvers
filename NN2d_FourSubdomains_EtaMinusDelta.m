function u = NN2d_FourSubdomains_EtaMinusDelta(f,eta,x,y,gl,gr,gb,gt,BCtype,pl,pr,pb,pt,uex,a,b,th,Nit,plot_iterates,plot_cvg_curve)
...
%     NDer23(1,Jx-idxa+3) = 0;
%     NDer23(1,Jx-idxa+2) = -1/h;
...
%     NDer34(1,Jy-idxb+3) = 0;
%     NDer34(1,Jy-idxb+2) = -1/h;
...
%     NDer41(end,end) = -1/h;
    ...
    for i=1:Nit
        ...
        if (BCtype == 0)
            ...
            g34(end) = gt(idxa);
            g41(1) = gl(idxb);
            % Compatibility at the cross-point
            g12(end) = 0.25*(g12(end)+g23(1)+g34(1)+g41(end));
            g41(end) = g12(end);
            g23(1) = g12(end);
            g34(1) = g12(end);
            % Then solve for all subdomains
            u1 = Solve2d_EtaMinusDelta(f1,eta,xmin,xa,ymin,yb,gl1,g12,gb1,g41,0,pl1,pr1,pb1,pt1);
            u2 = Solve2d_EtaMinusDelta(f2,eta,xa,xmax,ymin,yb,g12,gr2,gb2,g23,0,pl2,pr2,pb2,pt2);
            u3 = Solve2d_EtaMinusDelta(f3,eta,xa,xmax,yb,ymax,g34,gr3,g23,gt3,0,pl3,pr3,pb3,pt3);
            u4 = Solve2d_EtaMinusDelta(f4,eta,xmin,xa,yb,ymax,gl4,g34,g41,gt4,0,pl4,pr4,pb4,pt4);
        else
            u1 = Solve2d_EtaMinusDelta(f1,eta,xmin,xa,ymin,yb,gl1,lv*g12,gb1,lv*g41,1,pl1,lv*ones(idxb,1),pb1,lv*ones(idxa,1));
            u2 = Solve2d_EtaMinusDelta(f2,eta,xa,xmax,ymin,yb,lv*g12,gr2,gb2,lv*g23,1,lv*ones(idxb,1),pr2,pb2,lv*ones(Jx-idxa+1,1));
            u3 = Solve2d_EtaMinusDelta(f3,eta,xa,xmax,yb,ymax,lv*g34,gr3,lv*g23,gt3,1,lv*ones(Jy-idxb+1,1),pr3,lv*ones(Jx-idxa+1,1),pt3);
            u4 = Solve2d_EtaMinusDelta(f4,eta,xmin,xa,yb,ymax,gl4,lv*g34,lv*g41,gt4,1,pl4,lv*ones(Jy-idxb+1,1),lv*ones(idxa,1),pt4);
        end
...
%         du1dn12 = -NDer12*[u1(:,end-1);u1(:,end)]-0.5*h*[0; f1(2:end-1,end); 0];
%         du2dn21 = -NDer12*[u2(:,2);u2(:,1)]-0.5*h*[0; f2(2:end-1,1); 0];
        
        if (BCtype == 1)
            du1dn12(1) = du1dn12(1)-gb(idxa)-0.5*h*f1(1,end);
            du2dn21(1) = du2dn21(1)-gb(idxa)-0.5*h*f2(1,1);
        end 
        dpsidn12 = du1dn12+du2dn21;
...
%         du2dn23 = -NDer23*[u2(end-1,:)';u2(end,:)']-0.5*h*[0; f2(end,2:end-1)'; 0];
%         du3dn32 = -NDer23*[u3(2,:)';u3(1,:)']-0.5*h*[0; f3(1,2:end-1)'; 0];
        
        if (BCtype == 1)
            du2dn23(end) = du2dn23(end)-gr(idxb)-0.5*h*f2(end,end);
            du3dn32(end) = du3dn32(end)-gr(idxb)-0.5*h*f3(1,end);
        end 
        dpsidn23 = du2dn23+du3dn32;
...
%         du3dn34 = -NDer34*[u3(:,2);u3(:,1)]-0.5*h*[0; f3(2:end-1,1); 0];
%         du4dn43 = -NDer34*[u4(:,end-1);u4(:,end)]-0.5*h*[0; f4(2:end-1,end); 0];

        if (BCtype == 1)
            du3dn34(end) = du3dn34(end)-gt(idxa)-0.5*h*f3(end,1);
            du4dn43(end) = du4dn43(end)-gt(idxa)-0.5*h*f4(end,end);
        end 
        dpsidn34 = du3dn34+du4dn43;  
...
%         du4dn41 = -NDer41*[u4(2,:)';u4(1,:)']-0.5*h*[0; f4(1,2:end-1)'; 0];
%         du1dn14 = -NDer41*[u1(end-1,:)';u1(end,:)']-0.5*h*[0; f1(end,2:end-1)'; 0];

        if (BCtype == 1)
            du4dn41(1) = du4dn41(1)-gl(idxb)-0.5*h*f4(1,1);
            du1dn14(1) = du1dn14(1)-gl(idxb)-0.5*h*f1(end,1);
        end 
        dpsidn41 = du4dn41+du1dn14;  

        % Neumann step - compute the corrections psij for all subdomains
        % --------------------------------------------------------------
        if (BCtype == 0)
            psi1 = Solve2d_EtaMinusDelta(zeros(size(f1)),eta,xmin,xa,ymin,yb,zeros(idxb,1),dpsidn12,zeros(idxa,1),dpsidn41,1,lv*ones(idxb,1),zeros(idxb,1),lv*ones(idxa,1),zeros(idxa,1));
            psi2 = Solve2d_EtaMinusDelta(zeros(size(f2)),eta,xa,xmax,ymin,yb,dpsidn12,zeros(idxb,1),zeros(Jx-idxa+1,1),dpsidn23,1,zeros(idxb,1),lv*ones(idxb,1),lv*ones(Jx-idxa+1,1),zeros(Jx-idxa+1,1));
            psi3 = Solve2d_EtaMinusDelta(zeros(size(f3)),eta,xa,xmax,yb,ymax,dpsidn34,zeros(Jy-idxb+1,1),dpsidn23,zeros(Jx-idxa+1,1),1,zeros(Jy-idxb+1,1),lv*ones(Jy-idxb+1,1),zeros(Jx-idxa+1,1),lv*ones(Jx-idxa+1,1));
            psi4 = Solve2d_EtaMinusDelta(zeros(size(f4)),eta,xmin,xa,yb,ymax,zeros(Jy-idxb+1,1),dpsidn34,dpsidn41,zeros(idxa,1),1,lv*ones(Jy-idxb+1,1),zeros(Jy-idxb+1,1),zeros(idxa,1),lv*ones(idxa,1));
        else
            psi1 = Solve2d_EtaMinusDelta(zeros(size(f1)),eta,xmin,xa,ymin,yb,zeros(idxb,1),dpsidn12,zeros(idxa,1),dpsidn41,1,pl1,zeros(idxb,1),pb1,zeros(idxb,1));
            psi2 = Solve2d_EtaMinusDelta(zeros(size(f2)),eta,xa,xmax,ymin,yb,dpsidn12,zeros(idxb,1),zeros(Jx-idxa+1,1),dpsidn23,1,zeros(idxb,1),pr2,pb2,zeros(Jx-idxa+1,1));
            psi3 = Solve2d_EtaMinusDelta(zeros(size(f3)),eta,xa,xmax,yb,ymax,dpsidn34,zeros(Jy-idxb+1,1),dpsidn23,zeros(Jx-idxa+1,1),1,zeros(Jy-idxb+1,1),pr3,zeros(Jx-idxa+1,1),pt3);
            psi4 = Solve2d_EtaMinusDelta(zeros(size(f4)),eta,xmin,xa,yb,ymax,zeros(Jy-idxb+1,1),dpsidn34,dpsidn41,zeros(idxa,1),1,pl4,zeros(Jy-idxb+1,1),zeros(idxa,1),pt4);
        end
        ...
        % Update the gij
        g12 = g12-th*(psi1(:,end)+psi2(:,1));
        g23 = g23-th*(psi2(end,:)'+psi3(1,:)');
        g34 = g34-th*(psi3(:,1)+psi4(:,end));
        g41 = g41-th*(psi4(1,:)'+psi1(end,:)');
        
%         % Linear correction of the gij to make them continuous at the
%         % cross-point, imposing the average value of all gij (ie imposing
%         % no jump for the odd symmetric part)
%         avg = 0.25*(g12(end)+g23(1)+g34(1)+g41(end));
%         g12 = g12+(avg-g12(end))*(0:1/(idxb-1):1)';
%         g23 = g23+(avg-g23(1))*(1:-1/(Jx-idxa):0)';
%         g34 = g34+(avg-g34(1))*(1:-1/(Jy-idxb):0)';
%         g41 = g41+(avg-g41(end))*(0:1/(idxa-1):1)';
